...
router.get('/:forumId', auth, async (req, res) => {
  try {
    const forum = await Forum.findById(req.params.forumId)
      .populate('course', 'name code department professor')
...
router.put('/:forumId', auth, permissions(['professor', 'admin', 'root']), async (req, res) => {
  try {
    ...
    if (req.user.role === 'professor' && forum.course.professor.toString() !== req.user.id) {
      return res.status(403).json({
        success: false,
...
router.get('/:forumId/messages', auth, async (req, res) => {
  try {
    ...
    const forum = await Forum.findById(forumId).populate('course', 'professor');
