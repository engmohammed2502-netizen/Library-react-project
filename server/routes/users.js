...
router.put('/:id/lock', auth, permissions(['admin', 'root']), async (req, res) => {
  try {
    ...
    if (unlock) {
      ...
      // تجميد الحساب
      user.lockedUntil = new Date(Date.now() + parseInt(process.env.ACCOUNT_LOCKOUT_DURATION || 24 * 60 * 60 * 1000));
      user.isActive = false;
      ...
      await SecurityAlert.create({
        ...
        severity: 'medium',
        title: 'تجميد حساب',
        description: `تم تجميد حساب المستخدم: ${user.name || user.username}`,
        user: user._id,
        ipAddress: req.ip,
        userAgent: req.get('User-Agent'),
        metadata: {
          ...
          duration: process.env.ACCOUNT_LOCKOUT_DURATION || '24 hours'
        }
      });
    }
