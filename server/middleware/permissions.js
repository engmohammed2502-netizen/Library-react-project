...
const courseAccess = () => {
  return async (req, res, next) => {
    try {
      ...
      const course = await Course.findById(courseId).select('department professor isActive');
...
const forumAccess = () => {
  return async (req, res, next) => {
    try {
      ...
      const forum = await Forum.findById(forumId).populate('course', 'department professor');
  ...
  return async (req, res, next) => {
    try {
      ...
      switch (modelName) {
        ...
        case 'Course':
          isOwner = document.professor.toString() === req.user.id;
          break;
