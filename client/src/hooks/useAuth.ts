...
export const useAuth = () => {
  ...
  const canAccess = (permission: string): boolean => {
    ...
    // صلاحيات الروت (zero)
    if ((role as any) === 'root') return true;
  ...
  const canAccessDepartment = (department: string): boolean => {
    ...
    if ((authContext.user.role as any) === 'root' || (authContext.user.role as any) === 'admin') {
      return true;
    }
    
    // الأساتذة يمكنهم الوصول لتخصصاتهم فقط
    if ((authContext.user.role as any) === 'professor') {
      return authContext.user.department === department;
    }
    
    // الطلاب يمكنهم الوصول لتخصصاتهم فقط
    if ((authContext.user.role as any) === 'student') {
      return authContext.user.department === department;
    }
    
    // الضيوف يمكنهم الوصول لكل التخصصات
    if ((authContext.user.role as any) === 'guest') {
      return true;
    }
  ...
  const canAccessCourse = (courseProfessorId?: string, courseDepartment?: string): boolean => {
    ...
    if ((role as any) === 'root' || (role as any) === 'admin') {
      return true;
    }
  ...
  const canModifyCourse = (courseProfessorId?: string): boolean => {
    ...
    if ((role as any) === 'admin') return true;
  ...
  const canPostInForum = (forumId?: string): boolean => {
    ...
    if ((role as any) === 'guest') return false;
    
    // باقي المستخدمين يمكنهم المشاركة
    return (role as any) !== 'guest';
  };
  ...
  const isGuestSessionExpired = (): boolean => {
    if (!authContext.user || (authContext.user.role as any) !== 'guest') {
      return false;
    }
