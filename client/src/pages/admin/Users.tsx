...
const AdminUsers = () => {
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesRole = roleFilter === 'all' || (user.role as any) === roleFilter;
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 2, textAlign: 'center' }}>
            <Typography variant="h4">{users.filter(u => (u.role as any) === 'student').length}</Typography>
            <Typography variant="body2" color="textSecondary">طالب</Typography>
          </Paper>
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <Paper sx={{ p: 2, textAlign: 'center' }}>
            <Typography variant="h4">{users.filter(u => (u.role as any) === 'professor').length}</Typography>
          ...
          <TableBody>
            {filteredUsers.map((user) => (
              <TableRow key={user.id} hover>
                ...
                <TableCell>
                  <Chip 
                    label={(user.role as any) === 'student' ? 'طالب' : 
                           (user.role as any) === 'professor' ? 'أستاذ' : 
                           (user.role as any) === 'admin' ? 'مدير' : 'مدير النظام'}
          ...
          <Grid container spacing={2} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12} md={6}>
              <TextField
                ...
                disabled={(userForm.role as any) === 'root'}
              >
